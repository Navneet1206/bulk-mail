...
const templates = [
  ...
  { id: 3, name: 'Basic 3', html: '<h3>{{subject}}</h3><p>{{message}}</p>', type: 'basic' },
  { id: 4, name: 'Basic 4', html: '<div><h1>{{subject}}</h1>{{message}}</div>', type: 'basic' },
  { id: 5, name: 'Basic 5', html: '<p>{{subject}} - {{message}}</p>', type: 'basic' },
  { id: 6, name: 'Premium 1', html: '<img src="cid:logo" /><h1>{{subject}}</h1><p>{{message}}</p>', type: 'premium' },
  { id: 7, name: 'Premium 2', html: '<img src="cid:logo" /><h2>{{subject}}</h2><div>{{message}}</div>', type: 'premium' },
  { id: 8, name: 'Premium 3', html: '<div><img src="cid:logo" />{{subject}} - {{message}}</div>', type: 'premium' },
  { id: 9, name: 'Premium 4', html: '<img src="cid:logo" /><p>{{subject}}</p><p>{{message}}</p>', type: 'premium' },
  { id: 10, name: 'Premium 5', html: '<img src="cid:logo" /><h3>{{subject}}</h3>{{message}}', type: 'premium' },
];
...
exports.sendEmails = async (req, res) => {
  ...
  if (!template || (template.type === 'premium' && user.plan.templateAccess !== 'premium')) {
    return res.status(403).json({ message: 'Invalid template or access denied' });
  }
  ...
  try {
    for (const email of emails) {
      ...
      if (template.type === 'premium' && logo) {
        mailOptions.attachments = [{ filename: 'logo.png', path: logo.path, cid: 'logo' }];
      }
